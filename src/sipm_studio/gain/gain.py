...
from sipm_studio.dsp.qpe_peak_finding import fit_peaks, guess_peaks, gaussian
...
def calculate_gain(
    ...
    qs = integrate_current(
        nice_wfs, lower_bound=start_idx, upper_bound=end_idx, sample_time=SAMPLE_TIME
    )
    ...
    n, bins, patches = plt.hist(
        qs, bins=1000, range=ramge, histtype="stepfilled", alpha=0.15
    )
    ...
    try:
        params, errors = fit_peaks(
            ...
            bins,
            [np.argmax(n)],
            [bins[0], BIN_STD * bins[1]],
            [np.amax(n)],
            fit_width=QPE_FIT_WIDTH,
        )
    except:
    ...
    sigma_guess = np.abs(
        params[0][2]
    )  # take the standard deviation as the minimum width of a qpe peak
    min_distance_guess = (
        SIGMA_DISTANCE * sigma_guess
    )  # take 3 times the standard deviation as the minimum distance to find peaks
    mind_width_guess = SIGMA_WIDTH * sigma_guess
    # Guess the rest of the peak locations
    try:
        peaks, peak_locs, amplitudes = guess_peaks(
            n, bins, MIN_QPE_HEIGHT, min_distance_guess, mind_width_guess
        )
    except:
    ...
    try:
        gauss_params, gauss_errors = fit_peaks(
            n, bins, peaks, peak_locs, amplitudes, fit_width=QPE_FIT_WIDTH
        )
    except:
    ...
    n, bins, patches = plt.hist(
        qs, bins=1000, range=ramge, histtype="stepfilled", alpha=0.5
    )
