...
class sipm: 
    def __init__(self, gain: float, DCR: float, P_xt: float, avg_xt_num: float, P_ap: float, cell_recovery: float, alpha: float, PDE: float):
        """
    ...
    def get_params(self): 
        ...
        """
        return self.gain, self.DCR,  self.P_xt, self.avg_xt_num, self.P_ap, self.cell_recovery, self.alpha, self.PDE


class synthetic_waveforms: 
    def __init__(self, num_wfs: int, wf_len: int, wf_dts: np.array[np.array, ...], wf_t0s: np.array, wf_num_xts: np.array[np.array, ...], wf_xt_t0s: np.array, wf_xt_dts: np.array[np.array, ...], wf_num_aps: np.array[np.array, ...], wf_num_xts_per_ap: np.array[np.array, ...], wf_ap_t0s: np.array[np.array, ...], wf_ap_dts: np.array, dcr_amps: np.array[np.array, ...], xt_amps: np.array[np.array, ...], ap_amps: np.array[np.array, ...]): 
        """
...
def simulate_t0s(num_wfs: int, wf_len: int, sipm: type[sipm], sigma_detector: float, sampling_rate = 1: float) -> type[synthetic_waveforms]: 
    """
    ...
    wf_num_xts = [] # an array of arrays, containing the number of cross talk discharges 

    wf_ap_t0s = [] # an array of arrays, of all positions of afterpulsing events 
    wf_ap_dts = [] # a flat array, of all of the afterpulse delay times, can check against it later
    wf_num_aps = [] # an array of arrays, containing the number of afterpulses from k-number of XT events, based on a binomial
    wf_num_xts_per_ap = [] # an array of arrays, if an afterpulse follows a cross talk event 
    ...
    waveform_class = synthetic_waveforms(num_wfs, wf_len, wf_dts, wf_t0s, wf_num_xts, wf_xt_t0s, wf_xt_dts, wf_num_aps, wf_num_xts_per_ap, wf_ap_t0s, wf_ap_dts, dcr_amps, xt_amps, ap_amps)
