...
", "bias": 54, "device_name": "sipm_1st", "vpp": 0.5, "start_idx": 50, "end_idx": 250 }
    ...
    for dictionary in json_file["input_files"]:
        ...
        out_form = (input_file, bias, dictionary["device_name"], dictionary["vpp"], dictionary["start_idx"], dictionary["end_idx"], output_file)
...
", "bias": 54.5, "device_name": "sipm_1st", "vpp": 0.5, "gain_file": "path/to/gain/file/gain_file.h5", "light_window_start_idx": 50, "light_window_end_idx": 250, "dark_window_start_idx": 4000, "dark_window_end_idx": 4200   }
    ...
    for dictionary in json_file["input_files"]:
        ...
        if (channel == 1) and (str(dictionary["device_name"]) not in ["apd", "apd_goofy"]):
            raise ValueError("Channel number does not agree that this is an APD")

        if (channel == 0) and (str(dictionary["device_name"]) not in ["sipm", "sipm_1st", "sipm_1st_low_gain"]):
        ...
        out_form = (input_file, bias, dictionary["device_name"], dictionary["vpp"], dictionary["gain_file"], dictionary["light_window_start_idx"], dictionary["light_window_end_idx"], dictionary["dark_window_start_idx"], dictionary["dark_window_end_idx"], output_file)
